...
class VideoEditorApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(outer, text="第一阶段：批量导入 · 自动缩放裁切 · 合并 · MP4 输出", font=("Segoe UI", 10)).pack(anchor="w", pady=(4, 18))
        ...
        ttk.Label(settings, text="视频比例：").grid(row=0, column=0, sticky="w")
        ...
        output = ttk.LabelFrame(outer, text="输出 MP4", padding=12)
        output.pack(fill="x", pady=(12, 0))
        ttk.Label(output, text="输出文件名：").grid(row=0, column=0, sticky="w")
        self.output_name = tk.StringVar(value="final_video.mp4")
        ttk.Entry(output, textvariable=self.output_name, width=38).grid(row=0, column=1, sticky="ew", padx=8)
        ttk.Button(output, text="选择输出位置", command=self.choose_output).grid(row=0, column=2, sticky="e")
        self.output_label = ttk.Label(output, text="尚未选择输出位置")
        self.output_label.grid(row=1, column=0, columnspan=3, sticky="w", pady=(8, 0))
        ...
        self.start_button = ttk.Button(bottom, text="▶ 开始剪辑并输出 MP4", command=self.start)
        ...
        self.open_button.pack(side="left", padx=(10, 0), ipady=8)

    def add_files(self) -> None:
        paths = filedialog.askopenfilenames(title="选择视频", filetypes=[("视频文件", "*.mp4 *.mov *.mkv *.avi *.webm *.m4v")])
        for path in paths:
    ...
    def choose_output(self) -> None:
        folder = filedialog.askdirectory(title="选择 MP4 输出文件夹")
        if folder:
            self.output_dir = Path(folder)
            self.output_label.configure(text=f"输出位置：{self.output_dir}")
    ...
    def start(self) -> None:
        ...
        if not name:
            messagebox.showerror("输入错误", "请输入输出文件名。")
            return
        if not name.lower().endswith(".mp4"):
            name += ".mp4"
        if any(c in name for c in '<>:"/\\|?*'):
            messagebox.showerror("输入错误", "输出文件名包含 Windows 不允许的字符。")
            return
        try:
        ...
        if output.exists() and not messagebox.askyesno("确认覆盖", f"文件已存在：\n{output.name}\n\n是否覆盖？"):
            return
        ...
        threading.Thread(target=self._run, args=(sources, mode, max_seconds, output), daemon=True).start()

    def _run(self, sources: list[Path], mode: str, max_seconds: float | None, output: Path) -> None:
        try:
            video_auto_editor.run_editor(sources, output, mode, max_seconds, progress_callback=lambda percent, index, name: self.after(0, self._progress, percent, index, len(sources), name))
    ...
    def _progress(self, percent: int, index: int, total: int, name: str) -> None:
        ...
        self.status.set("正在合并视频……" if name == "合并视频" else f"正在处理第 {index}/{total} 个：{name}")

    def _done(self, output: Path) -> None:
        self._set_running(False)
        self.progress["value"] = 100
        self.progress_label.configure(text="100%")
        self.status.set(f"输出完成：{output.name}")
        self.open_button.configure(state="normal")
        messagebox.showinfo("输出完成", f"MP4 已成功输出：\n{output}")

    def _error(self, text: str) -> None:
        self._set_running(False)
        self.status.set("输出失败，请检查 FFmpeg 和输入视频")
        messagebox.showerror("输出失败", text[-4000:] or "发生未知错误。")
